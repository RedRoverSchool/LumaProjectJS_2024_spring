class TopsWomenPage {
  ...
  locators = {
    getWomenTopsPageHeader: () =>
      this.page.getByRole("heading", { name: "Tops" }),
    getWomenMyWishListHeading: () =>
      this.page.getByRole("heading", { name: "My Wish List" }),
    getWomenMyWishListEmptyMessage: () =>
      this.page.locator("div.block.block-wishlist .empty"),
    getCategoryFilterOption: () =>
      this.page.getByRole("tab", { name: "Category" }),
    getFilterOptionJacketsLink: () =>
      this.page.getByRole("link", { name: "Jackets" }),
    getArrayAllItems: () =>
      this.page
        .locator(".products .product-items .product-item-link")
        .allTextContents(),
    getTextCategoryJacketItems: () =>
      this.page
        .locator(
          "#narrow-by-list > div.filter-options-item.allow.active > div.filter-options-content > ol > li:nth-child(1) > a > span"
        )
        .innerText(),
    getWomenTopsProductItemsCards: () =>
      this.page.locator(".product-item-info"),
    listWomenTopsAddToMyWishListButtons: () =>
      this.page.locator("a.action.towishlist"),

    //getCategoryShoppingOptions: () =>
    // this.page.getByRole("tab", { name: "Category" }),
    getTeesCategoryShoppingOptions: () =>
      this.page.locator("[aria-hidden='false'] .items > .item > a").nth(2),
    getSizeShoppingOptions: () => this.page.getByRole("tab", { name: "Size" }),
    getSSizeShoppingOptions: () =>
      this.page.locator("[data-role='content'] [option-label='S']"),
    getColorShoppingOptions: () =>
      this.page.getByRole("tab", { name: "Color" }),
    getPurpleColorShoppingOptions: () =>
      this.page.locator(
        ".filter-options [role='presentation'] [attribute-code='color'] [option-label='Purple']"
      ),
    getShoppingByFilterList: () => this.page.locator(".filter-value"),
    getItemsNameList: () => this.page.locator(".product-item-link"),
    getPurpleColorItem: () =>
      this.page.locator("[aria-label='Color'] [aria-label='Purple']"),
    getSSizeItem: () =>
      this.page.locator("[aria-label='Size'] [aria-label='S']"),
  };
  ...
  async getRandomWomenTopsAddToWishListButton(index) {
    const addToWishListButtons =
      await this.getAllWomenTopsAddToMyWishListButtons();
  ...
  //mine
  /* async clickCategoryShoppingOptions() {
    await this.locators.getCategoryShoppingOptions().click();
    return this;
  }*/
